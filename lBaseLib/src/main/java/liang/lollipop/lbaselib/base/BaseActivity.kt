...
open class BaseActivity: AppCompatActivity(),
     ...
     * 初始化时，默认使用共用的动画加载器
     */
    protected var loadCallback = LoadCenter.getLoader()
    ...
    protected fun onLoad(){
        if(rootView != null){
            loadCallback.onLoad(layoutInflater,rootView as ViewGroup)
        }
    }

    protected fun onLoadEnd(){
        loadCallback.onLoadEnd()
    }

    override fun onBackPressed() {
        if(loadCallback.onBackPressed()){
            super.onBackPressed()
        }
    }
